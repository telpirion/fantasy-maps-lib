...
def convert_fantasy_map_to_bounding_boxes(map_dict: dict) -> Tuple:
    ...
    After conversion, the bounding boxes can then be used as input for VertexAI
    training.
...
def convert_batch_predictions_to_training_data(
    json_data: dict, minimum_confidence_value: float = 0.5
) -> Union[dict, None]:
    ...
    try:
        ...
        if len(bboxes) == 0:
            ...
            return {"imageGcsUri": image_gcs_uri, "boundingBoxAnnotations": bboxes}
